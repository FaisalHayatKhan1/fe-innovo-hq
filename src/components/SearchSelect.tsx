...
const SearchSelectField = ({
  ...
  return (
    <div className={cn(`grid w-full  items-center gap-1.5 my-2 ${className}`)}>
      {label && (
        <Label
          className={cn(
        ...
        <div className={`${outerDivStyle}`}>
          <Input
            ...
            className={cn(
              ...
              } border h-[43px] pr-10 border-customGray ring-transparent  rounded-[3px] focus-visible:ring-transparent shadow-[0 4px 50px 0 rgba(0, 0, 0, 0.1)] placeholder:text-f14 ${
                !!error
              ...
              <ul className="max-h-[250px] min-w-full overflow-y-auto max-w-md py-1 rounded-[5px] my-2 bg-white dark:bg-customGray-secondary ">
                {dropDownVal &&
                  dropDownVal?.map((item: any) => (
                    ...
                    >
                      <h1 className="font-normal text-f14 ">{item?.name}</h1>
                    </li>
                  ))}
              </ul>
            ) : (
